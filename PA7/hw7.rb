...
class Point < GeometryValue
  ...
  def intersctVerticalLine vline
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(x1, y1, x2, y2)
        ...
        if x1 < x2 then self else LineSegment.new(x2, y2, x1, y1) end
    else
        if y1 < y2 then self else LineSegment.new(x2, y2, x1, y1) end
    end
  end
  def shift(dx, dy)
...
class Let < GeometryExpression
  ...
  def eval_prog env
    ext_env = [[@s, @e1.eval_prog(env)]] + env
